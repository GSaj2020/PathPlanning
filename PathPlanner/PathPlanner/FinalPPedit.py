...
class PathPlanner(ScriptedLoadableModule):
  ...
  def __init__(self, parent):
    ...
    self.parent.contributors = ["Neel Patel (King's College London)"]
...
This file was originally developed by Neel Patel (King's College London)
"""
...
class PathPlannerLogic(ScriptedLoadableModuleLogic):
  ...
  def run(self, inputVolume, inputObstacle1, inputObstacle2,entries, targets, enableScreenshots=0):
    ...
    ## Obtain all the target points that lay within the hippocampus volume, output fiducial file (hippotarget)
    pointPicker = PickPointsMatrix()
    pointPicker.run(inputVolume, entries, targets)
    # load file created
    hippotargets = slicer.util.getNode('hippotargets')

    ## Obtain all the Good Paths that avoid hitting any obstacle
    get_goodpaths = MakeGoodPaths()
    get_goodpaths.run(entries, hippotargets, inputObstacle1, inputObstacle2)
    ...
    get_bestpaths = MakeBestPath()
    get_bestpaths.run(entries, hippotargets, inputObstacle1, inputObstacle2)
...
class MakeMesh_MakeTree():
  def run(self, inputObstacle1, inputObstacle2):
    ...
    obb1 = vtk.vtkOBBTree()
    obb1.SetDataSet(mesh.GetOutput())
    obb1.BuildLocator()

    # Create Mesh of obstacle 2
    mesh = vtk.vtkMarchingCubes()
    mesh.SetInputData(inputObstacle2.GetImageData())
    mesh.SetValue(0, 0.5)
    mesh.Update()

    # Create Tree of obstacle 3
    obb2 = vtk.vtkOBBTree()
    obb2.SetDataSet(mesh.GetOutput())
    obb2.BuildLocator()
    return obb1, obb2
...
class MakeGoodPaths():
  def run(self, entries, hippotargets, inputObstacle1, inputObstacle2):
    ...
    vtk_mesh = MakeMesh_MakeTree()
    obb1, obb2 = vtk_mesh.run(inputObstacle1, inputObstacle2)
    ...
    for i in range(0, entries.GetNumberOfFiducials()):
      ...
      for j in range(0, hippotargets.GetNumberOfFiducials()):
        ...
        if (obb1.IntersectWithLine(entry, target, vtk.vtkPoints(), vtk.vtkIdList()) == 0):
          if (obb2.IntersectWithLine(entry, target, vtk.vtkPoints(), vtk.vtkIdList()) == 0):
            ...
            if ln_length < 50:
              lns.InsertNextCell(vtk.vtkLine())

    # Set point and lines to object Path
    Path.SetPoints(pts)
    Path.SetLines(lns)
...
class MakeBestPath():
  def run(self, entries, hippotargets, inputObstacle1, inputObstacle2):
    ...
    # Insert entry and target into points, to create line
    vtk.vtkLine().GetPointIds().SetId(0, points.InsertNextPoint(best_entry[0], best_entry[1], best_entry[2]))
    vtk.vtkLine().GetPointIds().SetId(1, points.InsertNextPoint(best_target[0], best_target[1], best_target[2]))
    lines.InsertNextCell(vtk.vtkLine())

    # Set point and lines to object BestPath
    BestPath.SetPoints(points)
    BestPath.SetLines(lines)
    # return Path as a node to be viewed in slicer
    pathNode = slicer.mrmlScene.AddNewNodeByClass('vtkMRMLModelNode', 'BestPath')
    pathNode.SetAndObserveMesh(BestPath)
...
class PathPlannerTest(ScriptedLoadableModuleTest):
  ...
  def runTest(self):
    ...
    """
    self.setUp()
    self.test_LoadData('/Users/neel/Desktop/NPPathPlanning/BrainParcellation')  # this is a hard coded path you will need to change for your system
    ...
    self.test_MakeMesh_MakeTree_Input_IsNot_None()
    self.test_MakeMesh_MakeTree_Mesh_IsNot_None()
    self.test_MakeMesh_MakeTree_OBB_IsNot_None()
    self.test_MakeMesh_MakeTree_CallFunctionMesh()
    self.test_MakeGoodPaths_Check_Intersection_Condition_doesnot_hit()
    self.test_MakeGoodPaths_Check_Intersection_Condition_does_hit()
    self.test_MakeGoodPaths_Check_LineLength_Condition()
    self.test_ComputeDistanceImageFromLabelMap_output_is_as_expected()
    self.test_FindBestPath_Call_ComputeDistanceImageFromLabelMap()
    self.test_FindBestPath_Check_Intersection_Condition_doesnot_hit()
    self.test_FindBestPath_Check_Intersection_Condition_does_hit()
    self.test_FindBestPath_Check_LineLength_Condition()
    self.test_FindBestPath_Check_points_on_line_are_created_and_variable_created_are_of_appropriate_length()
    self.test_FindBestPath_Check_best_entry_and_best_target()
  ...
  ### MakeMesh_MakeTree Test Function ###

  def test_MakeMesh_MakeTree_Input_IsNot_None(self):
  ...
  def test_MakeMesh_MakeTree_Mesh_IsNot_None(self):
  ...
  def test_MakeMesh_MakeTree_OBB_IsNot_None(self):
    self.delayDisplay("Starting test: MakeMeshTree IsNot None.")
    inputImage = slicer.util.getNode('r_hippo')
    # Create Mesh of obstacle 1
    mesh = vtk.vtkMarchingCubes()
    ...
    obb1.SetDataSet(mesh.GetOutput())
    obb1.BuildLocator()
    self.assertIsNotNone(obb1)
    self.delayDisplay('Test passed! obb tree created is not empty')

  def test_MakeMesh_MakeTree_CallFunctionMesh(self):
  ...
  def test_MakeGoodPaths_Check_Intersection_Condition_doesnot_hit(self):
    self.delayDisplay("Starting test: MakeGoodPaths Check Intersection Condition - not intersect")
  ...
  def test_MakeGoodPaths_Check_Intersection_Condition_does_hit(self):
    self.delayDisplay("Starting test: MakeGoodPaths Check Intersection Condition - intersect")
  ...
  def test_MakeGoodPaths_Check_LineLength_Condition(self):
    self.delayDisplay("Starting test: MakeGoodPaths Check Length Condition")
  ...
  def test_FindBestPath_Call_ComputeDistanceImageFromLabelMap(self):
    self.delayDisplay("Starting test: FindBestPath calls Function Compute Distance Image From Label Map.")
    inputObstacle1 = slicer.util.getNode('ventricles')
  ...
  def test_FindBestPath_Check_Intersection_Condition_doesnot_hit(self):
    self.delayDisplay("Starting test: FindBestPath Check Intersection Condition - not intersect")
  ...
  def test_FindBestPath_Check_Intersection_Condition_does_hit(self):
    self.delayDisplay("Starting test: FindBestPath Check Intersection Condition - intersect")
  ...
  def test_FindBestPath_Check_LineLength_Condition(self):
    self.delayDisplay("Starting test: FindBestPath Check Length Condition")
      ...
      for j in range(0, hippotargets.GetNumberOfFiducials()):
        ...
        if (obb1.IntersectWithLine(entry, target, vtk.vtkPoints(), vtk.vtkIdList()) == 0):
          if (obb2.IntersectWithLine(entry, target, vtk.vtkPoints(), vtk.vtkIdList()) == 0):
            ...
            if ln_length <= 50:
              self.assertLess(ln_length, 51)
    self.delayDisplay('Test passed! all possible lines candidates for bestpath are less than 50')

  def test_FindBestPath_Check_points_on_line_are_created_and_variable_created_are_of_appropriate_length(self):
    self.delayDisplay("Starting test: FindBestPath Check points on line are created/variable created are of appropriate length ")
  ...
  def test_FindBestPath_Check_best_entry_and_best_target(self):
    self.delayDisplay("Starting test: FindBestPath Check best_entry and best_target are outputted")
    ...
    self.assertEqual(len(best_entry), 3)
    self.assertEqual(type(best_entry), list)
    self.assertEqual(len(best_target), 3)
    self.assertEqual(type(best_target), list)
    self.delayDisplay('Test passed! best_entry and best_target are outputted correctly and of correct length')
...
class PathPlannerLogic(ScriptedLoadableModuleLogic):
  ...
  def run(self, inputVolume, inputObstacle1, inputObstacle2,entries, targets, enableScreenshots=0):
    ...
    # Obtain GoodPaths that avoid hitting and obstacle
    get_goodpaths = MakeGoodPaths()
    get_goodpaths.run(entries, hippotargets, inputObstacle1, inputObstacle2)
...
class PathPlannerTest(ScriptedLoadableModuleTest):
  ...
  def runTest(self):
    ...
    """
    self.setUp()
    self.test_LoadData('/Users/ginasajith/Desktop/BrainParcellation') # this is a hard coded path you will need to change for your system
    self.test_PathPlanner_TestOutsidePoints()
    self.test_PathPlanner_TestInsidePoints()
    self.test_PathPlanner_TestEmptyMask()
    self.test_PathPlanner_TestEmptyPoints()
    self.test_MakeMesh_MakeTree_TransMatrix_make_Input_IsNot_None()
    self.test_MakeMesh_MakeTree_TransMatrix_make_Mesh_IsNot_None()
    self.test_MakeMesh_MakeTree_TransMatrix_make_OBB_IsNot_None()
    self.test_MakeMesh_MakeTree_TransMatrix_run_Input_IsNot_None()
    self.test_MakeMesh_MakeTree_TransMatrix_run_CallFunctionMesh()
    self.test_MakeGoodPaths_Check_Intersection_Condition()
    self.test_MakeGoodPaths_Check_entries_target_size_is_valid_for_xyz()
  ...
  ### VTK_and_MakeMesh Test MAKE Function ###

  def test_MakeMesh_MakeTree_Input_IsNot_None(self):
    self.delayDisplay("Starting test:  Input IsNot None.")
    inputObstacle = slicer.util.getNode('ventriclesTest')
    self.assertIsNotNone(inputObstacle)
    self.delayDisplay('Test passed! inputObstacle is not empty for use in function')

  def test_MakeMesh_MakeTree_Mesh_IsNot_None(self):
    self.delayDisplay("Starting test: MakeMeshTree IsNot None.")
    inputImage = slicer.util.getNode('r_hippoTest')
    mesh = vtk.vtkMarchingCubes()
    mesh.SetInputData(inputImage.GetImageData())
    mesh.ComputeGradientsOn()
    mesh.SetValue(0, 1)
    mesh.Update()
    meshoutput = mesh.GetOutput()
    self.assertIsNotNone(meshoutput)
  ...
  def test_MakeMesh_MakeTree_CallFunctionMesh(self):
    ...
    MeshTree = MakeMesh_MakeTree()
    # make mesh/tree
    MeshOBBTree = MeshTree.run(inputObstacle)
    self.assertIsNotNone(MeshOBBTree)
    self.delayDisplay('Test passed! Function was called and gives an output')

  ### MakeGoodPaths Test ###

  def test_MakeGoodPaths_Check_Intersection_Condition(self):
    self.delayDisplay("Starting test: MakeGoodPaths Check Intersection Condition.")

    inputObstacle = slicer.util.getNode('ventriclesTest')
    entries = slicer.util.getNode('entries')
    hippotargets = slicer.util.getNode('targets')

    # Compute the thresholded output volume using the Threshold Scalar Volume CLI module
    vtk_mesh = MakeMesh_MakeTree()
    lines, Path, points, triangles, OBB_Tree, trianglePointsId = vtk_mesh.run(inputObstacle)
    for i in range(0, 1):
      entry = [0, 0, 0]
      entries.GetNthFiducialPosition(i, entry)
      for j in range(0, 1):
        ...
        self.assertEqual(OBB_Tree.IntersectWithLine(entry, target, triangles, None), 0)
        self.delayDisplay('Test passed! Line does not intersect with mesh')

  def test_MakeGoodPaths_Check_entries_target_size_is_valid_for_xyz(self):
    self.delayDisplay("Starting test: MakeGoodPaths Check entry/target input is of valid size.")
    ...
    # Compute the thresholded output volume using the Threshold Scalar Volume CLI module
    vtk_mesh = MakeMesh_MakeTree_TransMatrix()
    lines, Path, points, triangles, OBB_Tree, trianglePointsId = vtk_mesh.run(inputObstacle)
    for i in range(0, 1):
      entry = [0, 0, 0]
      entries.GetNthFiducialPosition(i, entry)
      for j in range(0, 1):
        ...
        self.delayDisplay('Test passed! inputs, 1 entry and 1 target size both have xyz only.')
